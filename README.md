...
机器人会自动将疫苗记录存储在您配置的飞书多维表格中。

配置说明请参考 `.env.example` 文件。

创建 `.env` 文件并设置以下环境变量：
- `VACCINE_BASE_TOKEN` - 飞书多维表格的 Base Token
- `VACCINE_TABLE_ID` - 飞书多维表格的 Table ID
- `VACCINE_USER_ID` - 您的飞书用户 ID（用于接收提醒消息）
